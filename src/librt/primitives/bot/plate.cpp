...
rt_bot_plate_to_vol(struct rt_bot_internal **obot, struct rt_bot_internal *bot, int round_outer_edges, int quiet_mode)
{
	...
	*obot = NULL;
	...
	for (size_t j = 0; j < sbot->num_vertices ; j++) {
	    sph_m.vertProperties.push_back(sbot->vertices[3*j]);
	    sph_m.vertProperties.push_back(sbot->vertices[3*j+1]);
	    sph_m.vertProperties.push_back(sbot->vertices[3*j+2]);
	}
	for (size_t j = 0; j < sbot->num_faces; j++) {
	    sph_m.triVerts.push_back(sbot->faces[3*j]);
	    sph_m.triVerts.push_back(sbot->faces[3*j+1]);
	    sph_m.triVerts.push_back(sbot->faces[3*j+2]);
	}
	...
	for (int j = 0; j < vert_cnt; j++) {
	    rcc_m.vertProperties.push_back(vertices[j][X]);
	    rcc_m.vertProperties.push_back(vertices[j][Y]);
	    rcc_m.vertProperties.push_back(vertices[j][Z]);
	}
	for (int j = 0; j < face_cnt; j++) {
	    rcc_m.triVerts.push_back(faces[3*j]);
	    rcc_m.triVerts.push_back(faces[3*j+2]);
	    rcc_m.triVerts.push_back(faces[3*j+2]);
	}
	...
	for (size_t j = 0; j < 6; j++) {
	    arb_m.vertProperties.push_back(pts[3*j]);
	    arb_m.vertProperties.push_back(pts[3*j+1]);
	    arb_m.vertProperties.push_back(pts[3*j+2]);
	}
	for (size_t j = 0; j < 8; j++) {
	    arb_m.triVerts.push_back(faces[3*j]);
	    arb_m.triVerts.push_back(faces[3*j+1]);
	    arb_m.triVerts.push_back(faces[3*j+2]);
	}
